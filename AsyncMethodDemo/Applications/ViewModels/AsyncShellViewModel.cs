...
{
    [Export]
    internal class AsyncShellViewModel : ViewModel<IShellView>, IShellViewModel
    {
        #region Fields

        private const string TYPE = "Async Method Demo";
        ...
        public AsyncShellViewModel(IShellView view)
            : base(view)
        {
            Title = TYPE;
        ...
        private async Task<FileInformation> GetFileInformationAsync(string path)
        {
            await Task.Delay(50);
        ...
        private async Task<int> PopulateFilesAync()
        {
            _inProgress = true;
            Files.Clear();
            foreach (var item in Directory.GetFiles(@"C:\Windows\Microsoft.NET\Framework\v4.0.30319"))
            {
                ...
                Files.Add(await GetFileInformationAsync(item));
        ...
        {
            int count = await PopulateFilesAync();
