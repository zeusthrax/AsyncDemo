...
//Invoke(() => { Files.Clear(); });
//Invoke(() => { Files.Add(GetFileInformation(item)); });
//Invoke(() => { Title = string.Format("{0} - Totals Files: {1}", TYPE, Files.Count); });
//Task.Factory.StartNew<int>(PopulateFiles).ContinueWith((t) => {  Title = string.Format("{0} - Total Files: {1}", TYPE, t.Result); });
...
{
    [Export]
    internal class TaskShellViewModel : ViewModel<IShellView>, IShellViewModel
    {
        #region Fields

        private const string TYPE = "Task Demo";
        ...
        public TaskShellViewModel(IShellView view)
            : base(view)
        {
            Title = TYPE;
        ...
        private int PopulateFiles()
        {
            ...
            foreach (var item in Directory.GetFiles(@"C:\Windows\Microsoft.NET\Framework\v4.0.30319"))
            {
                ...
                //Title = string.Format("{0} - Totals Files: {1}", TYPE, Files.Count);
        ...
        {
            var t=Task.Factory.StartNew<int>(PopulateFiles);
            Title = string.Format("{0} - Total Files: {1}", TYPE, t.Result);
        }
